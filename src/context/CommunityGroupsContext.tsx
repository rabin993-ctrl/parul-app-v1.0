...
type CommunityCategory = Database['public']['Enums']['community_category_enum'];
...
export function CommunityGroupsProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateAdminSettings = useCallback((communityId: string, patch: Partial<CommunityAdminSettings>) => {
    ...
    supabase.rpc('update_community_settings', {
      ...
      ...(patch.guidelines != null ? { p_guidelines: patch.guidelines } : {}),
      ...(patch.defaultCategory != null ? { p_default_category: patch.defaultCategory as CommunityCategory } : {}),
      ...(patch.enabledTopics != null ? { p_enabled_topics: patch.enabledTopics as CommunityCategory[] } : {}),
      ...(patch.requirePhotoLostFound != null ? { p_require_photo_lost_found: patch.requirePhotoLostFound } : {}),
      ...(patch.showLocation != null ? { p_show_location: patch.showLocation } : {}),
    }).then(({ error }) => {
      if (error && __DEV__) {
        console.warn('[CommunityGroupsContext] update_community_settings failed:', error.message);
      }
    });
  }, []);
