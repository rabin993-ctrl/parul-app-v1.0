...
export function useAdoptionRequests() {
  ...
  const completeAdoption = useCallback((requestId: string, note?: string) => {
    ...
    // Optimistic local update — adopt the chosen request, reject the other
    // active requests for the SAME listing only.
    setRequests(prev => prev.map(r => {
    ...
    // Persist status changes to DB — scope the rejection to THIS listing so we
    // don't reject the poster's applicants on their other listings.
